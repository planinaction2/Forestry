...
class QuizApp:
    ...
    def init_ui(self):
        ...
        self.header_container = ft.Container(
            ...
            content=ft.Row([
                ft.Column([
                    ft.Text("Forestry App", size=24, weight=ft.FontWeight.BOLD),
    ...
    def setup_game(self):
        try:
            ...
            # Load the preformatted data
            self.quiz_df = self._load_quiz_data(self.raw_df, seed=seed)
    ...
    def load_question(self, idx):
        ...
        if show_answers:
            ...
            if pd.isna(explanation_text): explanation_text = "No explanation provided."
    ...
    def _load_quiz_data(self, raw_df, seed=None):
        """
        Parses the specific CSV format:
        Q.No, Question, Option A, Option B, Option C, Option D, Answer, Explanation
        """
        required_cols = ["Question", "Option A", "Option B", "Option C", "Option D", "Answer", "Explanation"]
        
        # Check if columns exist
        for col in required_cols:
            if col not in raw_df.columns:
                raise ValueError(f"Missing required column: {col}. Found: {list(raw_df.columns)}")

        # Randomize question order if seed provided
        if seed is not None:
        ...
        for _, row in shuffled.iterrows():
            # Parse answer column. It might be "(C) Anthrax" or just "C"
            raw_ans = str(row["Answer"]).strip()
            
            # Extract just the letter A, B, C, or D
            # Regex looks for A, B, C, or D inside parens OR at start of string
            match = re.search(r'\(?([A-D])\)?', raw_ans, re.IGNORECASE)
            clean_ans = match.group(1).upper() if match else "A" # Default to A if parsing fails, or handle error
